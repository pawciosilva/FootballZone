...
export default function UpcomingMatches() {
  ...
  useEffect(() => {
    ...
    //only for test
    const fetchData = async () => {
      const result = await getFixtures(date, dateWeek, undefined).catch((err) => {
        console.error(err);
      });
      if (result.data.result) {
                ...
                {matches.reverse().map((match, index) => (
                  ...
                  >
                    ...
                    <td className="text-center fw-bold align-middle">
                      <img class="upcoming-img mx-2" src={match.home_team_logo} alt="logo"></img>
                      {match.event_home_team} - {match.event_away_team}
                      <img class="upcoming-img mx-2" src={match.away_team_logo} alt="logo"></img>
                    </td>
